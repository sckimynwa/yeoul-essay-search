...
export default function Home() {
  ...
  const [chunks, setChunks] = useState<YCChunk[]>([]);
  ...
  const handleSearch = async () => {
    ...
    const results: YCChunk[] = await searchResponse.json();
  ...
  const handleSave = () => {
    ...
    localStorage.setItem("YC_KEY", apiKey);
    localStorage.setItem("YC_MATCH_COUNT", matchCount.toString());
    localStorage.setItem("YC_MODE", mode);
  ...
  const handleClear = () => {
    localStorage.removeItem("YC_KEY");
    localStorage.removeItem("YC_MATCH_COUNT");
    localStorage.removeItem("YC_MODE");
  ...
  useEffect(() => {
    const YC_KEY = localStorage.getItem("YC_KEY");
    const YC_MATCH_COUNT = localStorage.getItem("YC_MATCH_COUNT");
    const YC_MODE = localStorage.getItem("YC_MODE");
    ...
    if (YC_MATCH_COUNT) {
      setMatchCount(parseInt(YC_MATCH_COUNT));
    }

    if (YC_MODE) {
      setMode(YC_MODE as "search" | "chat");
    }
  ...
  return (
    <>
      <Head>
        <title>YEOUL-1</title>
        <meta name="description" content={`Yeoul's second brain`} />
            ...
            {apiKey.length === 51 ? (
              <div className="relative w-full mt-4">
                ...
                <input
                  ...
                  type="text"
                  placeholder="Good Day Sir! How can i assist you?"
                  value={query}
